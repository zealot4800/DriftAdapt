...
INFLUXDB_HOST = os.getenv("CARAVAN_INFLUX_HOST", "localhost")
INFLUXDB_PORT = int(os.getenv("CARAVAN_INFLUX_PORT", "8086"))
INFLUXDB_DATABASE = os.getenv("CARAVAN_INFLUX_DATABASE", "INFL_DB")
...
base_model_weights_path = Path(os.getenv("CARAVAN_INITIAL_MODEL", "initial_model_weights.pt"))
if not base_model_weights_path.is_file():
    raise FileNotFoundError(
        f"Initial model not found: {base_model_weights_path}. "
        "Set CARAVAN_INITIAL_MODEL to its absolute path."
    )
...
labeler_dnn_weights_path = Path(os.getenv("CARAVAN_LABELER_MODEL", "labeler_dnn.pt"))
if not labeler_dnn_weights_path.is_file():
    raise FileNotFoundError(
        f"Labeler model not found: {labeler_dnn_weights_path}. "
        "Set CARAVAN_LABELER_MODEL to its absolute path."
    )
